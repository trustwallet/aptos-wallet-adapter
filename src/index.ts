...
interface TrustWalletInterface {
  aptos?: PluginProvider;
}

interface TrustWindow extends Window {
  trustwallet?: TrustWalletInterface;
}
...
export class TrustWallet implements AdapterPlugin {
  ...
  trustwallet: TrustWalletInterface | undefined =
    typeof window !== "undefined" ? window.trustwallet : undefined;

  provider: PluginProvider | undefined =
    typeof this.trustwallet !== "undefined" ? this.trustwallet.aptos : undefined;
  ...
  async account(): Promise<AccountInfo> {
    ...
    if (!response) throw `${TrustWalletName} Account Error`;
    return response;
  }
  ...
  async signMessage(message: SignMessagePayload): Promise<SignMessageResponse> {
    try {
      if (typeof message !== "object" || !message.nonce) {
        `${TrustWalletName} Invalid signMessage Payload`;
      }
      const response = await this.provider?.signMessage(message);
      if (response) {
        return response;
      } else {
        throw `${TrustWalletName} Sign Message failed`;
      }
    } catch (error: any) {
      const errMsg = error.message;
      throw errMsg;
  ...
  async network(): Promise<NetworkInfo> {
    try {
      ...
      if (!response) throw `${TrustWalletName} Network Error`;
      return {
